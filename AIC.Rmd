...
## On inference after model selection {.build}
...
<div>
*There is an abundance of advice on how to perform the mechnics of choosing a model, .... There is a dearth of respectable theory, or even trustworthy advice, ...*
<div class="quote">
...
## AIC for Gaussian regression (fixed variance) {.build}
...
Consider $\xi = X \beta$ for $X$ and $n \times p$ matrix and $\beta \in \mathbf{R}^p$.  
...
## Model weights and model averaging {.build}
...
## Second take home message {.build}
...
## AIC as a prediction error estimate {.build}
...
## Forward stepwise variable selection {.build}
...
## Generalized information criteria {.build}
